            ...
            if threshold:
                ...
                if breached:
                    alerts_generated = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (o.get("officer_id") or o.get("id") for o in available) if resource_id],
            ...
            if threshold:
                ...
                if breached:
                    alerts_generated = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (o.get("officer_id") or o.get("id") for o in armed_available) if resource_id],
            ...
            if threshold:
                ...
                if breached:
                    alerts_generated = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (v.get("vehicle_id") or v.get("id") for v in vehicle_available) if resource_id],
            ...
            if threshold:
                ...
                if breached:
                    alerts_generated = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (v.get("vehicle_id") or v.get("id") for v in vehicles if v.get("fuel_percentage", 0) < DEFAULT_VEHICLE_FUEL_THRESHOLD) if resource_id],
                ...
                for vehicle in vehicles:
                    ...
                    if current <= threshold["threshold_value"]:
                        ...
                        alert = self.evaluate_metric(
                            ...
                            [resource_id for resource_id in (vehicle.get("vehicle_id") or vehicle.get("id"),) if resource_id],
            ...
            for ammunition in self.ammunition_rows(org_id):
                if ammunition["quantity"] <= ammunition["threshold"]:
                    alert = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (ammunition.get("type"),) if resource_id],
            ...
            for equipment in self.tactical_rows(org_id):
                if equipment["available_quantity"] <= equipment["threshold"]:
                    alert = self.evaluate_metric(
                        ...
                        [resource_id for resource_id in (equipment.get("category"),) if resource_id],
